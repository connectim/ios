...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.isLoading) {
        ...
        // 更改tableView被包围得头部边距
        scrollView.contentInset = UIEdgeInsetsMake(REFRESH_HEAD_HEIGHT + 64, 0, 0, 0);//头部包围距离为偏移距离
...
- (void)startLoadingForScrollView:(UIScrollView *)scrollView {
    ...
    scrollView.contentInset = UIEdgeInsetsMake(REFRESH_HEAD_HEIGHT + 64, 0, 0, 0);//改变tableview头部被包围状态
...
//    [UIView setAnimationDidStopSelector:@selector(animationDidStop:finished:)];//动画结束完执行完更改头部视图得内容
    ...
    scrollView.contentInset = UIEdgeInsetsMake(64, 0, 0, 0);//恢复头部零距离包围tableview
    [self.arrowImgView layer].transform = CATransform3DMakeRotation(M_PI * 2, 0, 0, 1);
//    [UIView commitAnimations];
}
