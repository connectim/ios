...
- (void)createPlayer
{
    ...
    if (self.audioPlayer.isPlaying) {
        NSError *faildError = [NSError errorWithDomain:@"gjcf.AudioManager.com" code:-235 userInfo:@{@"msg": @"正在播放失败"}];
    ...
    if (!self.currentPlayAudioFile) {
        ...
        NSError *faildError = [NSError errorWithDomain:@"gjcf.AudioManager.com" code:-236 userInfo:@{@"msg": @"没有播放的文件"}];
        if (self.delegate && [self.delegate respondsToSelector:@selector(audioPlayer:didOccusError:)]) {
            [self.delegate audioPlayer:self didOccusError:faildError];
        }
    ...
    self.audioPlayer = [[AVAudioPlayer alloc]initWithContentsOfURL:[NSURL URLWithString:self.currentPlayAudioFile.tempWamFilePath] error:nil];
    ...
    /* 获取当前播放文件得时间总长度 */
    self.currentPlayAudioDuration = [self getLocalWavFileDuration:self.currentPlayAudioFile.tempWamFilePath];
    ...
    if (_isPlaying) {
        ...
        NSError *faildError = [NSError errorWithDomain:@"gjcf.AudioManager.com" code:-237 userInfo:@{@"msg": @"文件播放失败"}];
        if (self.delegate && [self.delegate respondsToSelector:@selector(audioPlayer:didOccusError:)]) {
            [self.delegate audioPlayer:self didOccusError:faildError];
        }
    }
...
{
    return self.currentPlayAudioFile.tempWamFilePath;
}
...
#pragma mark - Update the progress of the play
- (void)updatePlayingProgress:(NSTimer *)timer
{
    ...
    /* Update the volume waveform */
    [self updateSoundMouter:timer];

    /* 0.9 progress should be stopped because 1.0 is done in the completion of the method update */
