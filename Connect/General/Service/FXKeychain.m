...
- (id)FXKeychain_propertyListRepresentation
{
    ...
    for (id obj in self)
    {
        ...
        if (value) [copy objectAddObject:value];
    }
    return copy;
}
...
- (NSData *)dataForKey:(id)key
{
    ...
    {
		DDLogInfo(@"FXKeychain failed to retrieve data for key '%@', error: %ld", key, (long)status);
	}
...
- (BOOL)setObject:(id)object forKey:(id)key
{
    ...
    if (data)
    {
        ...
        {
            DDLogInfo(@"FXKeychain failed to store data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    else if (self[key])
    {
        ...
        if (status != errSecSuccess)
        {
            DDLogInfo(@"FXKeychain failed to delete data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    return YES;
...
- (id)objectForKey:(id)key
{
    ...
    if (data)
    {
        ...
        if (!object)
        {
             DDLogInfo(@"FXKeychain failed to decode data for key '%@', error: %@", key, error);
        }
        return object;
    }
    else
