...
- (NSArray*)pinyinInitialsArray{
    ...
    for (NSString *str in [self pinyinArray]) {
        if ([str length] > 0) {
            [array objectAddObject:[str substringToIndex:1]];
        }
    }
    return array;
}
...
- (BOOL)preIsInAtoZ{
    return [@"QWERTYUIOPLKJHGFDSAZXCVBNM" containsString:self] || [[@"QWERTYUIOPLKJHGFDSAZXCVBNM" lowercaseString] containsString:self];
}
