...
-(UIImage*)scaleToSize:(CGSize)size
{
    ...
    // And set it to the currently used context
    UIGraphicsBeginImageContext(size);
    
    // Draw a picture that changes the size
    [self drawInRect:CGRectMake(xPos, yPos, width, height)];
    
    // Create a changed image from the current context
    UIImage* scaledImage = UIGraphicsGetImageFromCurrentImageContext();
    
    // Make the current context out of stack
    UIGraphicsEndImageContext();
    
    // Return the new image after changing the size
    return scaledImage;
}
