...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(TZAlbumModel *))completion{
    ...
    if (iOS8Later) {
        ...
        for (PHAssetCollection *collection in smartAlbums) {
            if ([collection.localizedTitle isEqualToString:LMLocalizedString(@"Chat Camera Roll", nil)] || [collection.localizedTitle isEqualToString:LMLocalizedString(@"All Photos", nil)]) {
        ...
        [self.assetLibrary enumerateGroupsWithTypes:ALAssetsGroupAll usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
            ...
            if ([name isEqualToString:LMLocalizedString(@"Chat Camera Roll", nil)] || [name isEqualToString:LMLocalizedString(@"All Photos", nil)]) {
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<TZAlbumModel *> *))completion{
    ...
    if (iOS8Later) {
        ...
        for (PHAssetCollection *collection in smartAlbums) {
            ...
            if ([collection.localizedTitle isEqualToString:LMLocalizedString(@"Chat Camera Roll", nil)] || [collection.localizedTitle isEqualToString:LMLocalizedString(@"All Photos", nil)]) {
                [albumArr objectInsert:[self modelWithResult:fetchResult name:collection.localizedTitle] atIndex:0];
            } else {
                [albumArr objectAddObject:[self modelWithResult:fetchResult name:collection.localizedTitle]];
            }
        }
        ...
        for (PHAssetCollection *collection in albums) {
            PHFetchResult *fetchResult = [PHAsset fetchAssetsInAssetCollection:collection options:option];
            if (fetchResult.count < 1) continue;
            if ([collection.localizedTitle isEqualToString:LMLocalizedString(@"Chat My Photo Stream", nil)]) {
                if (albumArr.count) {
                    [albumArr objectInsert:[self modelWithResult:fetchResult name:collection.localizedTitle] atIndex:1];
                } else {
                    [albumArr objectAddObject:[self modelWithResult:fetchResult name:collection.localizedTitle]];
                }
            } else {
        ...
        [self.assetLibrary enumerateGroupsWithTypes:ALAssetsGroupAll usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
            ...
            NSString *name = [group valueForProperty:ALAssetsGroupPropertyName];
            if ([name isEqualToString:LMLocalizedString(@"Chat Camera Roll", nil)] || [name isEqualToString:LMLocalizedString(@"All Photos", nil)]) {
                [albumArr objectInsert:[self modelWithResult:group name:name] atIndex:0];
            } else if ([name isEqualToString:LMLocalizedString(@"Chat My Photo Stream", nil)]) {
                if (albumArr.count) {
                    [albumArr objectInsert:[self modelWithResult:group name:name] atIndex:1];
                } else {
                    [albumArr objectAddObject:[self modelWithResult:group name:name]];
                }
            } else {
                [albumArr objectAddObject:[self modelWithResult:group name:name]];
            }
...
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<TZAssetModel *> *))completion {
    ...
    if ([result isKindOfClass:[PHFetchResult class]]) {
        ...
        [fetchResult enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [photoArr objectAddObject:[TZAssetModel modelWithAsset:asset type:type timeLength:timeLength]];
        }];
        ...
        ALAssetsGroupEnumerationResultsBlock resultBlock = ^(ALAsset *result, NSUInteger index, BOOL *stop)  {
            ...
            if (!allowPickingVideo){
                [photoArr objectAddObject:[TZAssetModel modelWithAsset:result type:type]];
                return;
            }
            /// Allow picking video
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                ...
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                [photoArr objectAddObject:[TZAssetModel modelWithAsset:result type:type timeLength:timeLength]];
            } else {
                [photoArr objectAddObject:[TZAssetModel modelWithAsset:result type:type]];
            }
        };
...
- (BOOL)isAssetsArray:(NSArray *)assets containAsset:(id)asset {
    if (iOS8Later) {
        ...
        for (ALAsset *asset_item in assets) {
            [selectedAssetUrls objectAddObject:[asset_item valueForProperty:ALAssetPropertyURLs]];
        }
...
- (NSString *)getNewAlbumName:(NSString *)name {
    if (iOS8Later) {
        NSString *newName;
        if ([name rangeOfString:@"Roll"].location != NSNotFound)         newName = LMLocalizedString(@"Chat Camera Roll", nil);
        else if ([name rangeOfString:@"Stream"].location != NSNotFound)  newName = LMLocalizedString(@"Chat My Photo Stream", nil);
        else if ([name rangeOfString:@"Added"].location != NSNotFound)   newName = LMLocalizedString(@"Chat Recent Added", nil);
        else if ([name rangeOfString:@"Selfies"].location != NSNotFound) newName = LMLocalizedString(@"Selfies", nil);
        else if ([name rangeOfString:@"shots"].location != NSNotFound)   newName = LMLocalizedString(@"Chat Screen Shots", nil);
        else if ([name rangeOfString:@"Videos"].location != NSNotFound)  newName = LMLocalizedString(@"Chat Videos", nil);
        else if ([name rangeOfString:@"Chat Panoramas"].location != NSNotFound)  newName = LMLocalizedString(@"Chat Panoramas", nil);
        else if ([name rangeOfString:@"Chat Favorites"].location != NSNotFound)  newName = LMLocalizedString(@"Chat Favorites", nil);
        else newName = name;
        return newName;
    } else {
        return name;
