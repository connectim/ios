...
- (void)configBottomToolBar {
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        CGSize originalBSize = [LMLocalizedString(@"Chat Original", nil) sizeWithFont:[UIFont systemFontOfSize:13] constrainedToHeight:50];
        _originalPhotoButton.frame = CGRectMake(5, 0, originalBSize.width + 50, 44);
...
//        _originalPhotoButton.contentEdgeInsets = UIEdgeInsetsMake(0, -50, 0, 0);
        ...
        _originalPhotoLable.frame = CGRectMake(CGRectGetMaxX(_originalPhotoButton.frame), 0, 70, 44);
...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            [_tzImagePickerVc showAlertWithTitle:[NSString stringWithFormat:LMLocalizedString(@"Chat You can only select photos", nil),_tzImagePickerVc.maxImagesCount]];
            return;
        // 2. if not over the maxImagesCount / 如果没有超过最大个数限制
        } else {
            [_tzImagePickerVc.selectedModels objectAddObject:model];
            if (self.photos) {
                [_tzImagePickerVc.selectedAssets objectAddObject:_assetsTemp[_currentIndex]];
                [self.photos objectAddObject:_photosTemp[_currentIndex]];
            }
            if (model.type == TZAssetModelMediaTypeVideo) {
                [_tzImagePickerVc showAlertWithTitle:LMLocalizedString(@"Multi-select condition choose video, the default send video when images", nil)];
            }
        }
    } else {
