...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:LMLocalizedString(@"Common Cancel", nil) style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
    _showTakePhotoBtn = (([_model.name isEqualToString:LMLocalizedString(@"Chat Camera Roll", nil)] || [_model.name isEqualToString:@"Chat Camera Roll"] ||  [_model.name isEqualToString:LMLocalizedString(@"Chat All photos", nil)]) && tzImagePickerVc.allowTakePicture);
...
- (void)configBottomToolBar {
    ...
    CGSize previewBSize = [LMLocalizedString(@"Chat Preview", nil) sizeWithFont:[UIFont systemFontOfSize:16] constrainedToHeight:44];
    if (previewBSize.width >80) {
        previewBSize.width = 80;
    }
    _previewButton.frame = CGRectMake(10, 3, previewBSize.width, 44);
    [_previewButton addTarget:self action:@selector(previewButtonClick) forControlEvents:UIControlEventTouchUpInside];
    _previewButton.titleLabel.font = [UIFont systemFontOfSize:16];
    [_previewButton setTitle:LMLocalizedString(@"Chat Preview", nil) forState:UIControlStateNormal];
    [_previewButton setTitle:LMLocalizedString(@"Chat Preview", nil) forState:UIControlStateDisabled];
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        CGSize originalBSize = [LMLocalizedString(@"Chat Original", nil) sizeWithFont:[UIFont systemFontOfSize:16] constrainedToHeight:50];
        if (originalBSize.width > 50) {
            originalBSize.width = 50;
        }
        _originalPhotoButton.frame = CGRectMake(CGRectGetMaxX(_previewButton.frame), self.view.tz_height - 50, originalBSize.width + 50, 50);
        _originalPhotoButton.imageEdgeInsets = UIEdgeInsetsMake(0, -8, 0, 0);
        [_originalPhotoButton addTarget:self action:@selector(originalPhotoButtonClick) forControlEvents:UIControlEventTouchUpInside];
        _originalPhotoButton.titleLabel.font = [UIFont systemFontOfSize:16];
        [_originalPhotoButton setTitle:LMLocalizedString(@"Chat Original", nil) forState:UIControlStateNormal];
        [_originalPhotoButton setTitle:LMLocalizedString(@"Chat Original", nil) forState:UIControlStateSelected];
        ...
        _originalPhotoLable.frame = CGRectMake(CGRectGetMaxX(_originalPhotoButton.frame), 0, 60, 50);
    ...
    if (chinese) {
        _okButton.frame = CGRectMake(self.view.tz_width - 44 - 12, 3, 60, 44);
    }

    _okButton.titleLabel.font = [UIFont systemFontOfSize:16];
    [_okButton addTarget:self action:@selector(okButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [_okButton setTitle:LMLocalizedString(@"Link Send", nil) forState:UIControlStateNormal];
    [_okButton setTitle:LMLocalizedString(@"Link Send", nil) forState:UIControlStateDisabled];
    ...
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            if (photo) {
                ...
                if (weakSelf.isSelectOriginalPhoto) {
                    [photos replaceObjectAtIndex:i withObject:photo];
                } else{
                    NSData *data = UIImageJPEGRepresentation(photo,0.4);
            ...
            [weakSelf.navigationController dismissViewControllerAnimated:YES completion:^{
                tzImagePickerVc.pickerDelegate = nil;
            }];
        }];
    }
}
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                [tzImagePickerVc.selectedModels objectAddObject:model];
                [weakSelf refreshBottomToolBarStatus];
            } else {
                [tzImagePickerVc showAlertWithTitle:[NSString stringWithFormat:LMLocalizedString(@"Chat You can only select photos", nil),tzImagePickerVc.maxImagesCount]];
            }
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:LMLocalizedString(@"Chat When choosing pictures can not choose the video", nil)];
        } else {
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied) && iOS8Later) {
        // 无权限 做一个友好的提示
        UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"无法使用相机" message:@"请在iPhone的""设置-隐私-相机""中允许访问相机" delegate:self cancelButtonTitle:LMLocalizedString(@"Common Cancel", nil) otherButtonTitles:@"设置", nil];
        [alert show];
    } else { // 调用相机
...
- (void)checkSelectedModels {
    for (TZAssetModel *model in _models) {
        ...
        for (TZAssetModel *model in tzImagePickerVc.selectedModels) {
            [selectedAssets objectAddObject:model.asset];
        }
    ...
    [[TZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
        ...
        [[TZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<TZAssetModel *> *models) {
            ...
            if (tzImagePickerVc.sortAscendingByModificationDate) {
                ...
                [_models objectAddObject:assetModel];
            } else {
                assetModel = [models firstObject];
                [_models objectInsert:assetModel atIndex:0];
            }
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                assetModel.isSelected = YES;
                [tzImagePickerVc.selectedModels objectAddObject:assetModel];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            ...
            [assets objectAddObject:model.asset];
        }
    }
...
- (NSArray *)aapl_indexPathsForElementsInRect:(CGRect)rect {
    ...
    for (UICollectionViewLayoutAttributes *layoutAttributes in allLayoutAttributes) {
        ...
        [indexPaths objectAddObject:indexPath];
    }
    return indexPaths;
}
