...
- (instancetype)initWithImageUrls:(NSArray *)imageUrls
{
    ...
    for (NSString *url in imageUrls) {
        ...
        [imageModels objectAddObject:aImageModel];
    }
...
- (UIBarButtonItem *)rightSavePhotoItem
{
    ...
    NSString *bunldePath = GJCFMainBundlePath(@"BrowserResource.bundle");
...
- (void)downloadCompletion:(NSData *)fileData cacheState:(BOOL)finish withIndex:(NSInteger)index
{
    ...
    if (imageSize.width > DEVICE_SIZE.width) {
        CGFloat v = DEVICE_SIZE.width * 1.f / imageSize.width;
        imageSize.width = DEVICE_SIZE.width;
        imageSize.height = imageSize.height * v;
...
- (void)removeImageAtIndex:(NSInteger)index
{
    ...
    [self.imageModelArray removeObjectAtIndexCheck:index];
