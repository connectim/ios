...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {        
        ...
        [cell longPress:^(MSSBrowseCollectionViewCell *browseCell) {
            ...
            BOOL haveLocalImage = [[NSFileManager defaultManager] fileExistsAtPath:browseItem.bigImageLocalPath];
            
            if([[YYImageCache sharedCache] getImageForKey:browseItem.bigImageUrl] || haveLocalImage)
            {
...
- (void)tap:(MSSBrowseCollectionViewCell *)browseCell
{
    ...
    if(browseItem.smallImageView)
    {
        ...
        [GCDQueue executeInMainQueue:^{
            browseCell.alpha = 0;
        } afterDelaySecs:ANIMATION_DURATION * 0.7];
    }
    else
    {
        [UIView animateWithDuration:ANIMATION_DURATION animations:^{
...
- (void)longPress:(MSSBrowseCollectionViewCell *)browseCell
{
    ...
    _browseActionSheet = [[MSSBrowseActionSheet alloc]initWithTitleArray:@[LMLocalizedString(@"Wallet Save to Photos", nil)] cancelButtonTitle:LMLocalizedString(@"Common Cancel", nil) didSelectedBlock:^(NSInteger index) {
        __strong __typeof(weakSelf)strongSelf = weakSelf;
...
{    // 保存图片
    ...
    else if(index == 1)
    {
        ...
        [self showBrowseRemindViewWithText:LMLocalizedString(@"Set Copy picture address success", nil)];
    }
}
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    ...
    if(error)
    {
        text = LMLocalizedString(@"Set Save Failed", nil);
    }
    else
    {
        text = LMLocalizedString(@"Login Save successful", nil);
    }
